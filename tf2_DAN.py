...
class DAN(tf.keras.Model):
    def __init__(self):
        super().__init__()
        self.conv1 = keras.layers.Conv2D(activation=tf.nn.relu,
                                        filters=96,
                                        kernel_size = [11,11],
                                        padding='same',
                                        strides=4)
        self.pool1 = keras.layers.MaxPool2D(pool_size=[3,3],strides=2,padding='valid') 
        self.conv2 = keras.layers.Conv2D(activation=tf.nn.relu,
                                        filters=256,
                                        kernel_size = [5,5],
                                        padding='same',
                                        strides=1)
        self.pool2 = keras.layers.MaxPool2D(pool_size=[3,3],strides=2,padding='valid')
        self.conv3 = keras.layers.Conv2D(activation=tf.nn.relu,
                                        filters=384,
                                        kernel_size = [3,3],
                                        padding='same',
                                        strides=1)
        self.conv4 = keras.layers.Conv2D(activation=tf.nn.relu,
                                        filters=384,
                                        kernel_size = [3,3],
                                        padding='same',
                                        strides=1)
        self.conv5 = keras.layers.Conv2D(activation=tf.nn.relu,
                                        filters=256,
                                        kernel_size = [3,3],
                                        padding='same',
                                        strides=1)
...
class DataLoad(object):
    ...
    def get_train_test_set(self,split_ratio):
        ...
        train_data = tf.map_fn(self.load_and_preprocess_image,train_paths) 
        test_data = tf.map_fn(self.load_and_preprocess_image,test_paths)
    ...
    def shuffle_and_split_path_label(self,all_image_paths,all_image_labels,split_ratio):
        ...
        if split_ratio == 1.:
            ...
            train_paths,test_paths = all_image_paths[:split_ratio],all_image_paths[split_ratio:]
            train_labels,test_labels = all_image_labels[:split_ratio],all_image_labels[split_ratio:]
