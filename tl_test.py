...
#==============================test=====================================
logger.info('------------tl DAN test start----------------------------')
start_time = time.time()

target_testset = batch_image_load.get_batch_images(test_paths)

test_step(target_testset,test_labels)

template = 'target test num: {},test set Loss: {},test Accuracy: {}'
print(template.format(len(test_labels),
                        test_loss.result(),
                        test_accuracy.result()*100)
                        )
logger.info(template.format(len(test_labels),
                        test_loss.result(),
                        test_accuracy.result()*100)
                        )

target_trainset = batch_image_load.get_batch_images(train_paths)

test_step(target_trainset,train_labels)

template = 'target train num: {}, Loss: {},Accuracy: {}'
print(template.format(len(train_labels),
                        test_loss.result(),
                        test_accuracy.result()*100)
                        )
logger.info(template.format(len(train_labels),
                        test_loss.result(),
                        test_accuracy.result()*100)
                        )
